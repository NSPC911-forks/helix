...
where
    F: Fn(&mut RenderContext<'a>, Span<'a>) + Copy,
{
    let theme = context.editor.theme.clone();
    let icons = ICONS.load();
...
where
    F: Fn(&mut RenderContext<'a>, Span<'a>) + Copy,
{
    if get_doc(context).is_modified() {
        let icons = ICONS.load();
...
where
    F: Fn(&mut RenderContext<'a>, Span<'a>) + Copy,
{
    let head = get_doc(context).version_control_head().unwrap_or_default();
    if !head.is_empty() {
        let icons = ICONS.load();
