...
| `accept-inline-completion` | When an inline completion (ghost text) is visible, pressing tab will accept it instead of performing the normal `smart-tab` action. If no inline completion is visible, tab behaves as usual. | `false` |
